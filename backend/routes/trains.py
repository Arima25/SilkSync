...
@trains_bp.route("/api/trains/price_for_route", methods=["POST"])
async def price_for_route():
    ...
    try:
        ...
        logger.exception("Error in /api/trains/price_for_route")
        return jsonify({"error": str(e)}), 500


# Station Search
@trains_bp.route("/api/trains/stations/search", methods=["GET"])
def stations_search():
...
# Ticket Query
@trains_bp.route("/api/trains/tickets", methods=["GET"])
def tickets():
...
# Transfer query
@trains_bp.route("/api/trains/transfer", methods=["GET"])
def transfer():
    from_station = request.args.get("from_station")
    to_station = request.args.get("to_station")
...
# Train stops
@trains_bp.route("/api/trains/stops/<train_code>", methods=["GET"])
...
# China time
@trains_bp.route("/api/trains/current-time", methods=["GET"])
def current_time():
    try:
...
# Ticket Pricing
@trains_bp.route("/api/trains/price", methods=["GET"])
def ticket_price():
    from_station = request.args.get("from_station")
    to_station = request.args.get("to_station")
    train_date = request.args.get("train_date")
...
@trains_bp.route("/api/trains/train-no", methods=["GET"])
def train_no():
    train_code = request.args.get("train_code")
    from_station = request.args.get("from_station")
    to_station = request.args.get("to_station")
...
# Nearest Station
@trains_bp.route("/api/trains/nearest-stations", methods=["GET"])
...
# Train Route
@trains_bp.route("/api/trains/route", methods=["GET"])
def route():
    from_station = request.args.get("from_station")
    to_station = request.args.get("to_station")
    train_date = request.args.get("train_date")
